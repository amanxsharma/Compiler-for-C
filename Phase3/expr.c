...
/*******for phase 3 **************/
/*
LBL_STK push_label(LBL_STK node,char *label_name)
{
	
	LBL_STK ret=(LBL_STK) malloc(sizeof(lbl_stk_node));
	ret->lbl_name=label_name;
	ret->next=NULL;
...
LBL_STK push_case_label(LBL_STK node,char *case_lbl,int val)
{
	BOOLEAN flag = FALSE;
	if(node!=NULL)
	{	if(node->con_type==SWITCH_ST)
			{
				CS_LBL temp=node->cl;
				temp=push_cslbl_list( temp,case_lbl,val);
				node->cl=temp;
				flag = TRUE;
			}
			else
...
LBL_STK push_default_label(LBL_STK node,char *default_label)
{
	...
	if(node!=NULL)
	{
			if(node->con_type==SWITCH_ST)
				{
					CS_LBL temp=node->cl;
					temp=push_default_list( temp,default_label);								
					node->cl=temp;
				}
				else
				{
...
{
	CS_LBL temp_list = list;
	BOOLEAN flag = FALSE;
	while(temp_list!=NULL)
	{
		if(temp_list->const_val == val)
		{
			error("duplicate value in case label: %d",val);
			flag = TRUE;
			break;
		}
		temp_list = temp_list->prev_lbl;
	}
	if(flag == FALSE)
	{
		
		CS_LBL ret=(CS_LBL)malloc(sizeof(cs_lbl_l));
...
{
	LBL_STK temp=stack;
	CS_LBL temp_cslist=NULL;
	
	while(temp!=NULL)
	{
		if(temp->con_type==SWITCH_ST)
		{
			temp_cslist=temp->cl;
			break;
		}
	...
	{	
			...
			while(temp_cslist!=NULL)
			{
				if(temp_cslist->const_val == -1)
				{}
				else 
				{				
				b_dispatch (B_EQ, TYSIGNEDINT , temp_cslist->const_val, temp_cslist->label,TRUE);
				
				}
				temp_cslist=temp_cslist->prev_lbl;
			}
